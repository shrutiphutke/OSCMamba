...
class AttentionBlockWithMLP(nn.Module):  
    def __init__(self, in_channels, reduction=2):  
        ...
        self.fc = nn.Sequential(
            nn.BatchNorm2d(1),
            nn.Conv2d(in_channels=1,out_channels=in_channels//reduction,kernel_size=3,stride=1,padding=1),
            nn.BatchNorm2d(in_channels//reduction),
            nn.ReLU(),
            nn.Conv2d(in_channels=in_channels//reduction, out_channels=in_channels, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(in_channels),
            nn.ReLU(),
            nn.Conv2d(in_channels=in_channels, out_channels=in_channels//reduction, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(in_channels//reduction),
            nn.ReLU(),
            nn.Conv2d(in_channels=in_channels//reduction, out_channels=1, kernel_size=1, stride=1),
            nn.Sigmoid()
        ) 
    def forward(self, x):  
        # print(x.shape)
        ...
        y = self.avg_pool(x1).view(b,1,h,w) 
...
class OSS_FFN(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        ...
        input_first = input
        x = input_first + self.drop_path(self.self_attention(self.ln_1(input_first)))
        input_next = x.permute(0,3,1,2).contiguous()
        input_next = self.ffn(input_next)
        x = x.permute(0,3,1,2).contiguous()      
        output = input_next+x
